...
class Hyperparameters:
    ...
    slot_batch_seqs = int(os.environ.get("SLOT_BATCH_SEQS", 64))
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float,
                 qk_gain_init: float, gated_attention: bool = False, value_residual: bool = False):
        super().__init__()
        if dim % num_heads != 0:
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, model_dim: int, num_heads: int,
                 ...
                 qk_gain_init: float, mtp_num_heads: int = 0, mtp_loss_weight: float = 0.1,
    ...
    def forward_with_delta(self, input_ids: Tensor, target_ids: Tensor, delta: Tensor) -> Tensor:
        """Forward pass with SLOT delta added after embedding + smear."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
        x = F.rms_norm(x, (x.size(-1),))
        x = self.smear(x)
        x = x + delta  # SLOT delta injection
        x0 = x
        v0 = None
        skips: list[Tensor] = []
        ve_cache: dict = {}
    ...
    def forward_logits_with_delta(self, input_ids: Tensor, delta: Tensor) -> Tensor:
        """Forward pass returning logits, with SLOT delta added after embedding + smear."""
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
...
def eval_val_sliding_slot(
    ...
    device: torch.device, val_tokens: Tensor, base_bytes_lut: Tensor,
    has_leading_space_lut: Tensor, is_boundary_token_lut: Tensor,
    stride: int, batch_seqs: int = 32, slot_steps: int = 8,
    slot_lr: float = 0.01, log0=print,
) -> tuple[float, float]:
    """Score-first SLOT: Legal variant of Stochastic Latent Optimization at Test-time.

    Like legal TTT, scores each chunk BEFORE using it for optimization:
    1. Score chunk N with current delta (from optimizing on chunk N-1)
    2. THEN optimize delta on chunk N's tokens
    3. Use that delta when scoring chunk N+1
    First chunk scored with delta=0 (no peeking). Last chunk scored but not trained on.
    """
    seq_len = args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    ...
    chunk_starts = list(range(0, total_tokens - seq_len + 1, seq_len))
    if not chunk_starts or chunk_starts[-1] + seq_len < total_tokens:
        chunk_starts.append(max(0, total_tokens - seq_len))
    total_chunks = len(chunk_starts)
    my_s = (total_chunks * rank) // world_size
    my_e = (total_chunks * (rank + 1)) // world_size
    my_chunks = chunk_starts[my_s:my_e]
    ...
    log0(f"slot_scorefirst:start chunks={len(my_chunks)} "
         f"slot_steps={slot_steps} slot_lr={slot_lr}")
    t0 = time.perf_counter()

    # Current delta — carries over from previous chunk's optimization
    # Initialized to zero for the first chunk (cold start)
    current_delta = None

    for ci, ws in enumerate(my_chunks):
        end = min(ws + seq_len, total_tokens)
        wlen = end - ws
        chunk = val_tokens[ws:end + 1].to(dtype=torch.int64, device=device)
        ...
        # STEP 1: SCORE this chunk with current delta (from previous chunk)
        # First chunk gets delta=0 (no peeking)
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            if current_delta is not None:
                logits = base_model.forward_logits_with_delta(x, current_delta[:1, :wlen, :])
            else:
                logits = base_model.forward_logits_with_delta(
                    x, torch.zeros(1, wlen, model_dim, device=device, dtype=torch.bfloat16))

        nll = F.cross_entropy(
            logits.reshape(-1, logits.size(-1)).float(),
            y.reshape(-1), reduction="none").reshape(1, wlen)
        ...
        # STEP 2: THEN optimize delta on this chunk (for use on NEXT chunk)
        # Skip optimization on last chunk (nothing to apply it to)
        if ci < len(my_chunks) - 1:
            delta = torch.zeros(1, seq_len, model_dim,
                                device=device, dtype=torch.bfloat16, requires_grad=True)
            slot_opt = torch.optim.AdamW([delta], lr=slot_lr, weight_decay=0.0)
            for step in range(slot_steps):
                slot_opt.zero_grad()
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    loss = base_model.forward_with_delta(x, y, delta[:, :wlen, :])
                loss.backward()
                slot_opt.step()
            current_delta = delta.detach()

        if rank == 0 and ((ci + 1) % 500 == 0 or ci == len(my_chunks) - 1):
            elapsed = time.perf_counter() - t0
            rl = loss_sum.item() / max(token_count.item(), 1)
            rbpb = rl / math.log(2.0) * (token_count.item() / max(byte_count.item(), 1)) if token_count.item() > 0 else 0.0
            log0(f"  slot_chunk [{ci+1}/{len(my_chunks)}] bpb={rbpb:.6f} time={elapsed:.1f}s")
    ...
    val_loss = (loss_sum / token_count).item()
    val_bpb = val_loss / math.log(2.0) * (token_count.item() / byte_count.item())
    log0(f"slot_scorefirst:done val_loss={val_loss:.6f} val_bpb={val_bpb:.6f} "
         f"elapsed={time.perf_counter() - t0:.1f}s")
    return val_loss, val_bpb
...
def _rebank_state_dict(sd: dict[str, Tensor], num_layers: int, tpl: dict[str, Tensor]) -> dict[str, Tensor]:
    out: dict[str, Tensor] = {}
    n = num_layers
    qo = [None] * (2 * n)
    kv = [None] * (2 * n)
...
@torch.compiler.disable
def gptq_quantize_weight(W: Tensor, H: Tensor, nb: int = 6, bs: int = 128,
                          damp: float = 0.01) -> tuple[Tensor, Tensor]:
    rows, cols = W.shape
    W = W.float().clone()
    H = H.float().clone()
    mv = 2 ** (nb - 1) - 1
    ...
    if dm < 1e-10:
        ...
        qi = torch.clamp(torch.round(W / sc.float()[:, None]), -mv - 1, mv).to(torch.int8)
        return qi, sc
    d = damp * dm
    H.diagonal().add_(d)
    try:
...
def collect_layer_hessians(
    ...
    seq_len: int = 2048, n_batches: int = 32,
) -> dict[str, Tensor]:
    model.eval()
    nl = model.num_layers
    n = nl
    ...
    def _layer(i: int, x: Tensor, x0: Tensor, xb: Tensor,
               vc: dict, v0: Tensor | None, bsz: int) -> tuple[Tensor, Tensor | None]:
        ...
        q = F.linear(ai, qw.to(ai.dtype)).reshape(bsz, seq_len, attn.num_heads, attn.head_dim)
        k = F.linear(ai, kw.to(ai.dtype)).reshape(bsz, seq_len, attn.num_kv_heads, attn.head_dim)
...
def gptq_quantize_model(
    model: nn.Module, hessians: dict[str, Tensor],
    n_bits: int = 6, blocksize: int = 128, dampening: float = 0.01,
...
def main() -> None:
    ...
    if args.gptq_enabled:
        ...
        hessians = collect_layer_hessians(
            base_model, val_tokens, device, seq_len=args.train_seq_len, n_batches=args.gptq_n_batches)
        torch.cuda.synchronize()
        ...
        quant_result, quant_meta, gptq_cnt, fallback_cnt = gptq_quantize_model(
            base_model, hessians, n_bits=6, blocksize=args.gptq_blocksize, dampening=args.gptq_dampening)
        torch.cuda.synchronize()
    ...
    if args.slot_enabled:
        # SLOT can run on the TTT-adapted model (if TTT ran) or on the base eval model
        slot_target_model = eval_model
        torch.cuda.synchronize()
        t_slot = time.perf_counter()
        slot_loss, slot_bpb = eval_val_sliding_slot(
            args, slot_target_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            stride=args.eval_stride, batch_seqs=args.slot_batch_seqs,
            slot_steps=args.slot_steps, slot_lr=args.slot_lr, log0=log0)
        torch.cuda.synchronize()
