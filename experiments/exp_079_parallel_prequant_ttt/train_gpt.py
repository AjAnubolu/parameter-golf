...
class Hyperparameters:
    ...
    prequant_ttt_epochs = int(os.environ.get("PREQUANT_TTT_EPOCHS", 9))
    prequant_ttt_lr = float(os.environ.get("PREQUANT_TTT_LR", 3e-4))
    ...
    lbfgs_slot_enabled = bool(int(os.environ.get("LBFGS_SLOT_ENABLED", "1")))
...
def pre_quant_adamw_ttt(h, device, val_data, base_model):
    ...
    for epoch in range(h.prequant_ttt_epochs):
        ...
        for ci in range(rank, num_chunks, world_size):
            ...
            # Cosine LR schedule based on GLOBAL chunk index
            cos_lr = h.prequant_ttt_lr * 0.5 * (1.0 + math.cos(math.pi * ci / max(num_chunks - 1, 1)))
            for pg in optimizer.param_groups:
                pg["lr"] = cos_lr
            ...
            if rank == 0 and ((ci + 1) % 5 == 0 or ci >= num_chunks - world_size):
                log(f"prequant_ttt:epoch {epoch+1}/{h.prequant_ttt_epochs} "
                    f"chunk {ci+1}/{num_chunks} lr={cos_lr:.6f}")
