...
class Hyperparameters:
    ...
    # Pre-quant AdamW TTT: run TTT on full-precision EMA model BEFORE GPTQ
    ...
    prequant_ttt_lr = float(os.environ.get("PREQUANT_TTT_LR", 2e-4))
    prequant_ttt_freeze_blocks = int(os.environ.get("PREQUANT_TTT_FREEZE_BLOCKS", 2))
    prequant_ttt_wd = float(os.environ.get("PREQUANT_TTT_WD", 0.01))
...
# --- Pre-quant AdamW TTT ---
# Key insight (PR #1364): run AdamW TTT on the full-precision EMA model BEFORE GPTQ,
# not SGD TTT on the quantized model after. This avoids the +0.030 BPB penalty from
# SGD TTT on quantized weights and yields -0.027 BPB gain.
...
def pre_quant_adamw_ttt(
    ...
    Score-first protocol: for each chunk, first score it (inference), then train on it.
    This ensures every token is scored before any gradient update that uses it.
    Runs on rank 0 only (single GPU) since this is a post-training adaptation step.
    """
    seq_len = args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    chunk_size = args.prequant_ttt_chunk_tokens
    num_epochs = args.prequant_ttt_epochs
    base_lr = args.prequant_ttt_lr
    freeze_n = args.prequant_ttt_freeze_blocks
    batch_seqs = args.prequant_ttt_batch_seqs

    num_chunks = (total_tokens + chunk_size - 1) // chunk_size

    log0(f"prequant_ttt:start epochs={num_epochs} lr={base_lr} wd={args.prequant_ttt_wd} "
         f"chunks={num_chunks} chunk_tokens={chunk_size} freeze_blocks={freeze_n} "
         f"batch_seqs={batch_seqs} grad_clip={args.prequant_ttt_grad_clip}")

    # Freeze first N encoder blocks
    frozen_block_ids = set(range(min(freeze_n, len(model.blocks))))
    ttt_params = []
    ...
    log0(f"prequant_ttt:params unfrozen={n_unfrozen} frozen={n_frozen}")

    # AdamW optimizer (the key difference from post-quant SGD TTT)
    optimizer = torch.optim.AdamW(
        ttt_params,
        lr=base_lr,
        betas=(0.9, 0.999),
        eps=1e-8,
        weight_decay=args.prequant_ttt_wd,
        fused=True,
    )
    ...
    for epoch in range(num_epochs):
        ...
        for ci in range(num_chunks):
            ...
            # --- Phase 1: SCORE this chunk (inference, no gradients) ---
            model.eval()
            with torch.inference_mode():
                chunk_seqs = (chunk_end - chunk_start) // seq_len
                if chunk_seqs > 0:
            ...
            if not is_last_chunk:
                ...
                if chunk_seqs > 0:
                    ...
                    cos_lr = base_lr * 0.5 * (1.0 + math.cos(
                        math.pi * (epoch * num_chunks + ci) / max(num_epochs * num_chunks - 1, 1)
                    ))
                    for pg in optimizer.param_groups:
                        pg['lr'] = cos_lr
        ...
        elapsed = time.perf_counter() - t0
        log0(f"  prequant_ttt epoch [{epoch+1}/{num_epochs}] "
             f"avg_loss={avg_loss:.6f} avg_bpb={avg_bpb:.6f} time={elapsed:.1f}s")
    ...
    elapsed = time.perf_counter() - t0
    log0(f"prequant_ttt:done total_time={elapsed:.1f}s")
...
def main() -> None:
    ...
    # --- Pre-quant AdamW TTT: adapt full-precision EMA model before GPTQ ---
    if args.prequant_ttt_enabled:
        # Destroy process group before long-running single-GPU TTT to avoid NCCL timeout
        if distributed:
            dist.barrier()
            dist.destroy_process_group()
            log0("prequant_ttt:destroyed process group to avoid NCCL timeout")
        if master_process:
            log0("prequant_ttt:starting AdamW TTT on full-precision EMA model")
            torch.cuda.synchronize()
            t_pqttt = time.perf_counter()
            pre_quant_adamw_ttt(
            ...
            torch.cuda.synchronize()
            log0(f"prequant_ttt:finished time={1000.0 * (time.perf_counter() - t_pqttt):.0f}ms")
        # Reinitialize process group and broadcast adapted weights
        if distributed:
            dist.init_process_group(backend="nccl", device_id=device)
            log0(f"prequant_ttt:rank {rank} reinitialized process group")
            for p in base_model.parameters():
                dist.broadcast(p.data, src=0)
            log0("prequant_ttt:weights broadcast to all ranks")
