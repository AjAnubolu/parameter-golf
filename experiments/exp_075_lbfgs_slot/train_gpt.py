...
class Hyperparameters:
    ...
    lbfgs_slot_batch_seqs = int(os.environ.get("LBFGS_SLOT_BATCH_SEQS", 1))
...
# --- L-BFGS Causal SLOT (logit-space) ---

@torch.compiler.disable
def eval_val_sliding_lbfgs_slot(
    args: Hyperparameters,
    base_model: nn.Module,
    rank: int,
    world_size: int,
    ...
    """L-BFGS Causal SLOT: optimize a logit-space delta per sliding window.

    Key design (from PR #1350):
    - Delta is a [vocab_size] vector added to ALL logit positions (broadcast)
    - L-BFGS optimizer with 25 iterations, history_size=20 for powerful 2nd-order optimization
    - Focal loss: only optimize on the last `focal_len` tokens of the scored region
    - Warm-start: delta from previous window initializes next window
    - Delta clamped to +/-5 to prevent divergence
    - Causal: score the window first with current delta, THEN optimize for next window
    """
    seq_len = args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    vocab_size = args.vocab_size
    focal_len = args.lbfgs_slot_focal
    max_iter = args.lbfgs_slot_iters
    history_size = args.lbfgs_slot_history
    clamp_val = args.lbfgs_slot_clamp
    slot_lr = args.lbfgs_slot_lr
    ...
    # Freeze model parameters -- we only optimize the delta
    for p in base_model.parameters():
        p.requires_grad_(False)
    base_model.eval()

    log0(f"lbfgs_slot:start windows={len(my_windows)} max_iter={max_iter} "
         f"history={history_size} focal={focal_len} clamp={clamp_val} lr={slot_lr}")
    t0 = time.perf_counter()

    # Warm-start delta (carries over between windows)
    current_delta = torch.zeros(vocab_size, device=device, dtype=torch.float32)

    for wi, ws in enumerate(my_windows):
        end = min(ws + seq_len, total_tokens)
        wlen = end - ws
        chunk = val_tokens[ws:end + 1].to(dtype=torch.int64, device=device)
        x = chunk[:-1].unsqueeze(0)  # (1, wlen) padded to (1, seq_len)
        ...
        # Determine scored region for this window
        s = 0 if ws == 0 else max(wlen - stride, 0)

        # ---- STEP 1: SCORE with current delta (from previous window's optimization) ----
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            logits = base_model.forward_logits(x)  # (1, seq_len, V)
        # Add current delta to logits
        scored_logits = logits.float() + current_delta[None, None, :]  # broadcast (1,1,V) -> (1,T,V)
        ...
        # ---- STEP 2: OPTIMIZE delta on this window for NEXT window (causal) ----
        # We optimize on the focal region (last focal_len tokens of scored region)
        if wi < len(my_windows) - 1:
            # Detach logits for optimization (no grad through model)
            base_logits = logits.detach().float()  # (1, seq_len, V)

            # Focal region: last focal_len tokens of the actual window
            focal_start = max(0, wlen - focal_len)
            focal_end = wlen
            focal_targets = y[0, focal_start:focal_end]  # (focal_len,)
            focal_base_logits = base_logits[0, focal_start:focal_end, :]  # (focal_len, V)

            # Initialize delta from warm-start (clone so L-BFGS can modify)
            delta = current_delta.clone().detach().requires_grad_(True)
            ...
            def closure():
                optimizer.zero_grad()
                adjusted_logits = focal_base_logits + delta[None, :]  # (focal_len, V)
                loss = F.cross_entropy(adjusted_logits, focal_targets, reduction="mean")
                loss.backward()
                return loss
            ...
            # Clamp delta
            delta.data.clamp_(-clamp_val, clamp_val)
            current_delta = delta.detach()

        if rank == 0 and ((wi + 1) % 500 == 0 or wi == len(my_windows) - 1):
            elapsed = time.perf_counter() - t0
            rl = loss_sum.item() / max(token_count.item(), 1)
            rbpb = rl / math.log(2.0) * (token_count.item() / max(byte_count.item(), 1)) if token_count.item() > 0 else 0.0
            log0(f"  lbfgs_slot [{wi+1}/{len(my_windows)}] bpb={rbpb:.6f} time={elapsed:.1f}s")

    # Restore model parameter requires_grad
    for p in base_model.parameters():
        p.requires_grad_(True)
    ...
    base_model.eval()
    log0(f"lbfgs_slot:done val_loss={val_loss:.6f} val_bpb={val_bpb:.6f} "
         f"elapsed={time.perf_counter() - t0:.1f}s")
    return val_loss, val_bpb
...
def collect_layer_hessians(
    ...
    def _process_layer(i: int, x: Tensor, x0: Tensor, x_batch: Tensor,
        ...
        with torch.autocast(device_type="cuda", enabled=False):
            _accumulate_hessian(f"mlp_up_bank.{i}", mlp_input_f.T @ mlp_input_f)
        ...
        with torch.autocast(device_type="cuda", enabled=False):
            _accumulate_hessian(f"mlp_down_bank.{i}", mlp_down_input_f.T @ mlp_down_input_f)
...
def gptq_quantize_model(
    ...
    for i in range(n):
        ...
        weight_to_hessian[f"blocks.{i}.mlp.fc.weight"] = f"mlp_up_bank.{i}"
        weight_to_hessian[f"blocks.{i}.mlp.proj.weight"] = f"mlp_down_bank.{i}"
...
def main() -> None:
    ...
    # L-BFGS Causal SLOT (logit-space delta optimization, PR #1350)
    if args.lbfgs_slot_enabled:
        torch.cuda.synchronize()
        t_slot = time.perf_counter()
        slot_loss, slot_bpb = eval_val_sliding_lbfgs_slot(
            args, eval_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            stride=args.eval_stride, log0=log0,
        )
        torch.cuda.synchronize()
        log0(f"lbfgs_slot val_loss:{slot_loss:.4f} val_bpb:{slot_bpb:.4f} "
             f"eval_time:{1000.0 * (time.perf_counter() - t_slot):.0f}ms")
        log0(f"lbfgs_slot_exact val_loss:{slot_loss:.8f} val_bpb:{slot_bpb:.8f}")
    if distributed:
